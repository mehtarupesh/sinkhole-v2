...
export function useVaultSync(connections, otpValidator) {
      ...
      conn.on('data', async (data) => {
        try {
          ...
          if (msg.phase === 'offer') {
            ...
            const [categorization, accessOrder] = await Promise.all([getCategorization(), getAccessOrder()]);

            // Apply peer's tombstones first (removes locally-resurrected units before diffing),
            // then merge categories and access order.
            await Promise.all([mergeTombstones(msg.tombstones), mergeCategorization(msg.categorization), mergeAccessOrder(msg.accessOrder)]);

            // Fetch post-tombstone local units and tombstones to include in Message 2.
            const [localUnits, tombstones] = await Promise.all([getAllUnits(), getTombstones()]);
            ...
            if (conn.open) conn.send({ type: VAULT_CHANNEL, phase: 'transfer', units: toSend, want, categorization, accessOrder, tombstones });
            if (want.length === 0) {
            ...
            const [tombstonedUids, idRemap] = await Promise.all([
              mergeTombstones(msg.tombstones),
              mergeCategorization(msg.categorization),
            ]);
            await mergeAccessOrder(msg.accessOrder);
            // Exclude tombstoned units — they were hard-deleted on the peer and must not resurface.
            const { added, updated } = await mergeUnits((msg.units ?? []).filter((u) => !tombstonedUids.has(u.uid)), idRemap);
  ...
  const sync = useCallback(async (conn, otp) => {
    ...
    try {
      const [units, categorization, accessOrder, tombstones] = await Promise.all([getAllUnits(), getCategorization(), getAccessOrder(), getTombstones()]);
